...
// Function to create a histogram chart
function createHistogramChart(data) {
  // Extract the budget values from the data
  const budgets = data.map((d) => d.budget);
  
    // Define the number of bins for the histogram
    const numBins = 10;

  // Define the range of budget values for the histogram bins
  const budgetRange = [0, d3.max(budgets) + d3.max(budgets)/numBins]; 
  ...
  // Create x and y scales for the histogram chart
  const xScale = d3
      .scaleLinear()
      .domain(budgetRange)
      .range([0,   width]);
  ...
  // Append vertical bars using the histogram data and scales
  svg
      .selectAll(".bar")
      .data(bins)
      .enter()
  ...
  // Append x and y axes to the histogram chart
  svg
      .append("g")
      .attr("class", "x-axis")
      .attr("transform", `translate(0,${height})`)
